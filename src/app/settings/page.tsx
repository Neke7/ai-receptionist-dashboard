...
export default function SettingsPage() {
  ...
  // Surface the ?error=connect flag set by the /api/calendar/connect redirect
  // when the OAuth kickoff couldn't resolve the client. Read from the URL
  // directly (rather than useSearchParams) to avoid a Suspense boundary.
  const [connectError, setConnectError] = useState(false);
  useEffect(() => {
    if (typeof window === "undefined") return;
    const params = new URLSearchParams(window.location.search);
    setConnectError(params.get("error") === "connect");
  }, []);
